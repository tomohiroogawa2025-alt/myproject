...
OUT_DIR = Path("outputs/reports/07_classification_possibility_900_1700nm") / RUN_ID
...
all_spec_cols = spec_cols
all_wavelengths = wavelengths
all_nm = wavenumber_to_nm(all_wavelengths) if axis_unit == "cm-1" else all_wavelengths
range_mask = (all_nm >= SPECTRAL_RANGE_NM[0]) & (all_nm <= SPECTRAL_RANGE_NM[1])
spec_cols = [col for col, keep in zip(all_spec_cols, range_mask) if keep]
if not spec_cols:
    raise ValueError(f"No spectral columns found in range {SPECTRAL_RANGE_NM} nm")
wavelengths = np.array([float(c) for c in spec_cols])
selected_nm = wavenumber_to_nm(wavelengths) if axis_unit == "cm-1" else wavelengths
...
spectral_range_summary = pd.DataFrame(
    {
        "setting": ["all_detected", "selected_for_07"],
        "n_features": [len(all_spec_cols), len(spec_cols)],
        "axis_min": [float(np.nanmin(all_wavelengths)), float(np.nanmin(wavelengths))],
        "axis_max": [float(np.nanmax(all_wavelengths)), float(np.nanmax(wavelengths))],
        "nm_min": [float(np.nanmin(all_nm)), float(np.nanmin(selected_nm))],
        "nm_max": [float(np.nanmax(all_nm)), float(np.nanmax(selected_nm))],
    }
)
spectral_range_summary.to_csv(OUT_DIR / "spectral_range_summary.csv", index=False, encoding="utf-8-sig")
...
summary = {
    ...
    "requested_nm_min": float(SPECTRAL_RANGE_NM[0]),
    "requested_nm_max": float(SPECTRAL_RANGE_NM[1]),
    "selected_nm_min": float(np.nanmin(selected_nm)),
    "selected_nm_max": float(np.nanmax(selected_nm)),
    "all_nm_min": float(np.nanmin(all_nm)),
    "all_nm_max": float(np.nanmax(all_nm)),
    "axis_unit": axis_unit,
