...
REPORT_ROOT = Path("outputs/reports/07_classification_possibility_900_1700nm")
...
OUT_DOCX = latest_dir / "07_900-1700nm_分類可能性評価_上司報告.docx"
...
run = title.add_run("900-1700 nm近赤外スペクトルによる樹種・N/L材分類可能性評価")
...
add_body(
    doc,
    "900-1700 nm相当の近赤外スペクトル範囲に限定し、樹種13分類およびN/L材2分類の可能性を確認した。代表的な前処理・分類器による3-fold CVでは、両タスクとも非常に高い分類性能を示した。",
)
add_bullets(
    doc,
    [
...
add_body(
    doc,
    f"入力は train.csv のスペクトル列のみを特徴量として使用した。含水率列は特徴量に含めていない。指定範囲は{summary['requested_nm_min']:.0f}-{summary['requested_nm_max']:.0f} nmである。元データの実範囲は約{summary['all_nm_min']:.0f}-{summary['all_nm_max']:.0f} nmであるため、実際には交差部分の約{summary['selected_nm_min']:.0f}-{summary['selected_nm_max']:.0f} nm、{summary['n_spectral_features']:,}点を使用した。",
)
